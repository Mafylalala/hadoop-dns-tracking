...
import de.sec.dns.test.TestToolPhase2;
 ...
 */
public class DNSAnalysisTool2TwoPass extends Configured implements Tool {
	/**
	...
	private static final Log LOG = LogFactory
			.getLog(DNSAnalysisTool2TwoPass.class);
	...
	public static void main(String[] args) throws Exception {
		...
		ToolRunner.run(new DNSAnalysisTool2TwoPass(), args);
		// }
	}
	...
	public int run(String[] args) throws Exception {
		...
		doCrossValidationChildren = gBuilder.withOption(numClassesOption)
				.withOption(numInstancesPerClassOption).withOption(seedOption)
				.create();
		...
		Group trainTestChildren = gBuilder.withOption(testOnlyOption)
				.withOption(trainingOnlyOption).withOption(fromTimeOption)
				.withOption(toTimeOption).withOption(usePRCOption)
				.withOption(useRandomTrainingSessionsOption)
				.withOption(offsetBetweenTrainAndTestOption).withRequired(true)
				.withOption(dropAmbiguousResults).create();
		...
		Group options = gBuilder.withOption(basePathOption)
				...
				.withOption(createDataSetOption)
			...
			if (!cl.hasOption(trainingOnlyOption)) {
				if (!cl.hasOption(useRandomTrainingSessionsOption)) {
					...
					while (trainingSession.before(lastSession)) {
						...
						if (ToolRunner.run(conf, new TestToolPhase2(), null) == 0) {
							break;
						}
					 ...
					 * Util.DEFAULT_SESSION_OFFSET); // TODO Offset m�sste
					 * hier noch eingearbeitet werden
					 * 
					 ...
					 * == 0) { break; }
					 * 
					 * if (ToolRunner.run(conf, new TestToolPhase2(), null) ==
					 * 0) { break; }
					 * 
					 * if (ToolRunner.run(conf, new ConfusionMatrixTool(), null)
					 * == 0) { break; }
					 * 
					 * OccurenceMatrix occTraining = OccurenceMatrix
					 * .getOccurenceData(null, conf, Util
		...
		if (cl.hasOption(doAnalysisOption)) {
			...
			String jobName = Util.JOB_NAME + " ["
					+ DNSAnalysisTool2TwoPass.ACTION + "] {test="
