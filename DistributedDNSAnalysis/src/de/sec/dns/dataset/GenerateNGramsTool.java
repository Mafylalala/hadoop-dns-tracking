 ...
 */
public class GenerateNGramsTool extends Configured implements Tool {
	/**
	 * The action used by the job name.
	 */
	private static String ACTION = "GenerateNGrams";
	...
	public int run(String[] args) throws Exception {
		...
		outputPath = new Path(conf.get(Util.CONF_NGRAMS_PATH));
		...
		// set the jobname
		String jobName = Util.JOB_NAME + " [" + GenerateNGramsTool.ACTION
				+ "] {logdata=" + inputPath.getName() + ", header="
				+ ", ngrams=" + outputPath.getName() + ", session="
		...
		job.setJarByClass(GenerateNGramsTool.class);
		job.setMapperClass(GenerateNGramsMapper.class);
		job.setReducerClass(GenerateNGramsReducer.class);
