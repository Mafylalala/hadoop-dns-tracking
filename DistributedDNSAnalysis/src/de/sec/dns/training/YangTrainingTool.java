...
/**
 * An implementation of an hadoop {@link Tool} used to train instances using
 * lift or support.
 * 
 * @author Elmo Randschau
 */
public class YangTrainingTool extends Configured implements Tool {
	/**
	...
	public int run(String[] args) throws Exception {
		...
		// trainingdate will be appended via multipleoutputs in reducer

		Path inputPath = new Path(datasetPath + "/"
				+ conf.get(Util.CONF_SESSION_DURATION) + "/" + option + "/");

		String jobName = Util.JOB_NAME_YANG + " [" + YangTrainingTool.ACTION
		...
		job.setJarByClass(YangTrainingTool.class);
		job.setMapperClass(YangTrainingMapper.class);
		job.setReducerClass(YangTrainingReducer.class);
		...
		MultipleOutputs.addNamedOutput(job, "probOfClass",
